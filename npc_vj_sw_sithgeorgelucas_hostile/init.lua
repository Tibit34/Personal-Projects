...
-----------------------------------------------*/
ENT.Model = {"models/kaesar/saberalter/saberalter_npc.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = GetConVarNumber("vj_sw_sithgeorgelucas_hostile_h")
...
---------------------------------------------------------------------------------------------------------------------------------------------
ENT.VJ_NPC_Class = {"CLASS_CIS_HOSTILE","CLASS_DROIDS_HOSTILE","CLASS_STARWARS_CIS_HOSTILE","CLASS_VILLAINS_HOSTILE","CLASS_SITH_HOSTILE"} -- NPCs with the same class with be allied to each other
...
-- Melee Attack ---------------------------------------------------------------------------------------------------------------------------------------------
ENT.HasMeleeAttack = true -- Should the SNPC have a melee attack?
ENT.MeleeAttackDamage = GetConVarNumber("vj_sw_sithgeorgelucas_hostile_d")	
...
ENT.NextAnyAttackTime_Melee = 0.45 -- How much time until it can use any attack again? | Counted in Seconds
...
ENT.RangeAttackEntityToSpawn = "obj_vj_lightbolt" -- The entity that is spawned when range attacking
	-- ====== Animation Variables ====== --
...
ENT.RangeToMeleeDistance = 580 -- How close does it have to be until it uses melee?
...
ENT.LeapAttackDamage = GetConVarNumber("vj_sw_sithgeorgelucas_hostile_d")
...
ENT.LightsaberSpin_Damage = 1350
...
ENT.HasLightsaber = 1
ENT.LightsaberModel = "models/aoc_weapon/w_sword_01.mdl"

ENT.CanLightsaberThrow = true
ENT.LightsaberThrow_NextT = 0
ENT.LightsaberThrow_Nexttime = 2.5
...
ENT.LightsaberThrow_damage = 1350
...
ENT.LightsaberJump_AttackRange = 420
ENT.LightsaberJump_AttackRadius = 180
ENT.LightsaberJump_Damage = 1350
...
function ENT:CustomOnInitialize()
			...
			self.ExtraGunModel1:SetMaterial("models/alyx/emptool_glow")
...
if self:Health() < 90357 then
self:SetHealth(self:Health()+ math.random(7,8),0,90357) 
end
...
self.GodMode = true
self:EmitSound("shared/magicshit/magic_electric.wav.mp3", 90, 100, 1)
for x=1, 4 do
self:EmitSound("shared/magicshit/magic_electric.wav.mp3", 90, 100, 1)
self.scanner1 = ents.Create("obj_saber_lightening")
self.scanner1:SetPos(self:GetPos() +self:GetUp()*math.random(30,50) +self:GetForward()*15 +self:GetRight()*math.random(-20,20))
...
timer.Simple(math.random(3,5),function() if IsValid(self) then self.att4 = false end end)
end
end
end
end
...
--Saber's special ability
if self:GetEnemy() != nil  then

if self:GetPos():Distance(self:GetEnemy():GetPos()) < 2000 then
if self.att3 == false then
self.att3 = true
timer.Simple(50,function() if IsValid(self) then
for x=1, 150 do
timer.Simple(math.Rand(0,10),function() if IsValid(self) then
ParticleEffectAttach("striderbuster_break_lightning",PATTACH_POINT_FOLLOW,self,3)
...
timer.Simple(55,function() if IsValid(self) then
--ParticleEffectAttach("dragon_thunder", PATTACH_ABSORIGIN_FOLLOW, self, 0)
ParticleEffectAttach("dragon_thunder",PATTACH_POINT_FOLLOW,self,3)
end end)
timer.Simple(56,function() if IsValid(self) then
--ParticleEffectAttach("dragon_thunder", PATTACH_ABSORIGIN_FOLLOW, self, 0)
ParticleEffectAttach("dragon_thunder",PATTACH_POINT_FOLLOW,self,3)
end end)
timer.Simple(57,function() if IsValid(self) then
--ParticleEffectAttach("dragon_thunder", PATTACH_ABSORIGIN_FOLLOW, self, 0)
ParticleEffectAttach("dragon_thunder",PATTACH_POINT_FOLLOW,self,3)
end end)
timer.Simple(58,function() if IsValid(self) then
--ParticleEffectAttach("dragon_thunder", PATTACH_ABSORIGIN_FOLLOW, self, 0)
ParticleEffectAttach("dragon_thunder",PATTACH_POINT_FOLLOW,self,3)
end end)
timer.Simple(59,function() if IsValid(self) then
--ParticleEffectAttach("dragon_thunder", PATTACH_ABSORIGIN_FOLLOW, self, 0)
ParticleEffectAttach("dragon_thunder",PATTACH_POINT_FOLLOW,self,3)
end end)
...
self:VJ_ACT_PLAYACTIVITY("swing",true,3.3,false)
VJ_EmitSound(self,"elc/explosion_lv4.wav", 130,100)
ParticleEffect("skill_blast4",self:GetPos(),Angle(0,0,0),nil)
ParticleEffect("elc_strong_attackhull",self:GetPos(),Angle(0,0,0),nil)
timer.Simple(0,function() if IsValid(self) then
util.VJ_SphereDamage(self,self,self:GetPos()+self:GetUp()*math.random(25,35),10000,math.random(38746,41856),DMG_SHOCK,true,true)
end end)
